...
public class Utils
{
	...
	/**
	 * Retrieve the value from the properties file according to the key. If
	 * value is null, set to the default value.
	 * 
	 * @param key
	 *            - property key
	 * @param defaultVal
	 *            - default value if the key not present in config file
		...
		else
		{
			...
			catch (Exception e)
			{
				log.warn("getIntegerProperty(): cannot parse integer from properties file for: "
								+ key + "fail over to default value: "
								+ defaultVal, e);
